...
local function project_action(callback)
...
local find_files = function(project_path)
  local opt = {
    cwd = project_path,
...
local find_project_files = project_action(find_files)

local browse_project_files = project_action(function(project_path)
  local opt = {
    cwd = project_path,
    hidden = config.options.show_hidden,
  }
...
local search_in_project_files = project_action(function(project_path)
  local opt = {
    cwd = project_path,
    hidden = config.options.show_hidden,
    mode = "insert",
...
local recent_project_files = project_action(function(_)
  local opt = {
    cwd_only = true,
    hidden = config.options.show_hidden,
  }
...
local function projects(opts)
  ...
  if opts.default_action == nil then
    opts.default_action = find_files
  end

  local default = project_action(opts.default_action)
